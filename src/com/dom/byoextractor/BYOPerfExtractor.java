	...
	public void BYOPerfExterior(XSSFWorkbook wb) {
		...
		try {
			...
			if(BYOExtractorUtils.verticalTable[0]){
				...
				cell = performanceSheet.createRow(rowCount).createCell(0);
				performanceSheet.addMergedRegion(new CellRangeAddress(rowCount, rowCount, 0, 4));
				...
				int count =0;
				//System.out.println(rowCount);
				//Row tempHeader = performanceSheet.createRow(rowCount);
			...
			if(BYOExtractorUtils.verticalTable[0]){
				...
				while(l<25){
					...
					if(accTblContent!=null&&accTblContent.id()!=null&&!accTblContent.id().contains("likes-and-labels-container") && accTblContent != null){
